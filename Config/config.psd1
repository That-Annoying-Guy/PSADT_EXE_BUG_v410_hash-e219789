@{
    ...
    Toolkit = @{
        ...
        #LogPath = '$envLogs\Install'
        # LogPath = '$env:Logs\Install'
        ...
        # Automatically changes DeployMode to Silent during the OOBE. (Should have been called SilentDuringOOBE)
        OobeDetection = $true
    ...
    MergeNRollLogs = @{
        #MergeNRollLogs Module settings
        #Specify if the separate log file created to record log handling is kept after processing is done. 
        #Creates %temp%\$($adtSession.InstallName)_LogHandling_<Random>.log
		LogHandlingDebug = $true
        
        #Specify if the log files in the "LogToSubfolder" should be bundled together in a compressed zip file OR a single merged log file
        LogsAreCompressed = $true

        #If LogsAreCompressed is $true, Select between .Zip and .Log. .Log merges all logs to one indexed text-based log file.
		LogCompressFormat = '.Log'

        #Specify if existing Logfiles that would be overwritten, rename .log to .BK# where # is 1-9. .log is ALWAYS most recent.
        #   If log file are compressed to a ZIP file, rename .ZIP to .BK# where # is 1-9. .ZIP is ALWAYS most recent.
		RotateLogFiles = $true
    }
